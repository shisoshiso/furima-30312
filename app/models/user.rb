class User < ApplicationRecord
  ...
  with_options presence: true do
    ...
    validates :first_name, format: { with: /\A[ぁ-んァ-ン一-龥]+\z/, message: 'は全角（漢字・平仮名・カタカナ）での入力が必要です' }
    validates :last_name, format: { with: /\A[ぁ-んァ-ン一-龥]+\z/, message: 'は全角（漢字・平仮名・カタカナ）での入力が必要です' }
